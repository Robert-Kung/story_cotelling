...
class StoryBotRetellEnv:
    def __init__(self, story_summary_dataset: dict, dialogue_evalution_model_ckpt: str, kg2text_model_ckpt: str, embedding_model_name: str, device: str):
        # config
        self.device = device
    ...
    def step(self, action):
        ...
        if action==0:
            filtered_kg = self.action0()
        elif action==1:
            filtered_kg = self.action1()
        elif action==2:
            filtered_kg = self.action2()
        elif action==3:
            filtered_kg = self.action3()
        elif action==4:
            filtered_kg = self.action4()
        else:
        ...
        # if filtered_kg is not None and filtered_kg == []:
            filtered_kg = self.action5()
    ...
    def action0(self):
        ...
        # 選擇後續的劇情
        filtered_kg = self.current_story_kg.filter_kg_by_sentence_idx(None, None, None, -1, current_discussion_idx)[:3]
        return filtered_kg
    ...
    def action1(self):
        filtered_kg = []
        for fact in self.current_discussion_fact_list:
            filtered_kg = self.current_story_kg.filter_kg(fact.subject, None, None, -1, fuzzy=True)[:3]
            if filtered_kg != []:
                return filtered_kg
        return filtered_kg
    ...
    def action2(self):
        filtered_kg = []
        for fact in self.current_discussion_fact_list:
            filtered_kg = self.current_story_kg.filter_kg(None, fact.relation, None, -1, fuzzy=True)[:3]
            if filtered_kg != []:
                return filtered_kg
        return filtered_kg
    ...
    def action3(self):
        filtered_kg = []
        for fact in self.current_discussion_fact_list:
            filtered_kg = self.current_story_kg.filter_kg(None, None, fact.object, -1, fuzzy=True)[:3]
            if filtered_kg != []:
                return filtered_kg
        return filtered_kg
    ...
    def observation(self):
        ...
        candidate_response_kg_list = [self.action0(), self.action1(), self.action2(), self.action3(), self.action4()]
        candidate_response_list = self.kg2text_plus(candidate_response_kg_list)           
